...
for city in citys:
    ...
    number = 0
    for l in range(0, lenght, 30):
        number += 1
        detmir = fetch("https://api.detmir.ru/v2/products?filter=categories["
                       f"].alias:lego;promo:false;withregion:RU-{city}&expand=meta.facet.ages.adults,meta.facet.gender.adults,"
                       f"webp&meta=*&limit=30&offset={l}&sort=popularity:desc", {
                           "headers": {
                               "accept": "*/*",
                               "accept-language": "ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7",
                               "cache-control": "no-cache",
        ...
        print(f"page {number},items {l} saved")
    print(f"Finish {city}")
